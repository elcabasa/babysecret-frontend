...
export function RoutineSection() {
  ...
  return (
    <>
      {/* Mobile: Figma routine builder */}
      <section className="bg-white px-4 py-10 md:hidden">
        <div className="mx-auto max-w-[358px] text-center">
          <p className="font-secondary text-[11px] font-semibold uppercase leading-[14px] tracking-[0.55px] text-[#0055B8]">
            Routine builder
          </p>
          <h2 className="mt-2 text-2xl font-bold leading-[30px] tracking-[-0.24px] text-[#142F54]">
            Build your baby&apos;s routine.
          </h2>
          <p className="mt-2 text-[12px] leading-[18px] text-[#424753]">
              ...
              <button
                ...
                className={`rounded-full px-4 py-2 text-xs font-semibold transition ${
                  selected === name
                    ...
                    : "border border-[#E5E3E3] bg-white text-[#424753] hover:border-[#0055B8]"
                }`}
              >
                {name}
              </button>
          ...
          <div className="mt-6 rounded-xl glass-panel p-5 text-left shadow-[0_2px_12px_rgba(20,47,84,0.06)]">
            <h3 className="text-center text-xl font-bold text-[#142F54]">
            ...
            <div className="mt-5">
              {routines[selected].map(([step, name, price]) => (
                <div
                  key={step}
                  className="grid grid-cols-[70px_1fr_auto] items-center gap-3 border-b border-[#efebe2] py-4 text-sm"
                >
                  <span className="font-secondary text-xs font-semibold text-[#0055B8]">
                    {step}
                  </span>
                  <strong className="text-[#142F54]">{name}</strong>
                  <span className="text-[#424753]">{price}</span>
                </div>
              ))}
            </div>
            <div className="mt-5 flex items-center justify-between">
              <strong className="text-[#142F54]">Total</strong>
              <strong className="text-lg text-[#142F54]">
            ...
            <div className="mt-5 text-center">
              <button className="inline-flex items-center justify-center gap-2 rounded-full bg-[#0055B8] px-7 py-3 text-sm font-semibold text-white transition hover:bg-[#004a9f]">
      ...
      {/* Desktop: existing routine builder */}
      <section className="hidden bg-[#f9fcff] px-6 py-24 sm:px-10 md:block">
