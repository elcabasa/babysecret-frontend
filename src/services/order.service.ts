...
export async function validateCheckoutItems(items: CheckoutRequest["items"]) {
  ...
  for (const item of items) {
    ...
    if (updated.price !== item.price)
      priceChanges.push({ before: item, after: updated });
  }
  return {
...
export async function createDemoOrder(
  ...
  const validation = await validateCheckoutItems(input.items);
  if (!validation.valid)
    throw new Error("One or more products are unavailable.");
  const subtotal = validation.updatedItems.reduce(
